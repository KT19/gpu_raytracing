#ifndef SCENE_CORNELL_SMOKE
#define SCENE_CORNELL_SMOKE
...
__global__ void init_texture_on_device(texture* d_texture) {
    if(threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        d_texture[1].set_solid_color(color(.73f, .73f, .73f));
        //green
        ...
        d_texture[3].set_solid_color(color(7, 7, 7));
        //light green
        d_texture[4].set_texture_type(SOLID_COLOR);
        d_texture[4].set_solid_color(color(0.0f, 0.6f, 0.6f));
        //purple
        d_texture[5].set_texture_type(SOLID_COLOR);
        d_texture[5].set_solid_color(color(0.5f, 0, 0.5f));
    }
}
...
void init_materials_on_host(int num_objs, material* d_materials) {
    ...
    h_materials[cur_index++] = material(DIFFUSE, color(.65f, .05f, .05f), 0.0f, 1.0f); //red
    h_materials[cur_index++] = material(DIFFUSE, color(.73f, .73f, .73f), 0.0f, 1.0f); //white
    h_materials[cur_index++] = material(DIFFUSE, color(.12f, .45f, .15f), 0.0f, 1.0f); //green
    h_materials[cur_index++] = material(DIFFUSE_LIGHT, color(7, 7, 7), 0.0f, 1.0f); //diffuse_light
    h_materials[cur_index++] = material(ISOTROPIC, color(0, 0, 0), 0.0f, 1.0f);
    h_materials[cur_index++] = material(ISOTROPIC, color(1, 1, 1), 0.0f, 1.0f);
...
void init_world_on_host(int num_objs, primitive* d_primitives) {
    ...
    h_primitives[cur_index].set_primitive_type(QUAD);
    h_primitives[cur_index++].set_quad(point3(113, 554, 127), vec3(330, 0, 0), vec3(0, 0, 305));
    
    h_primitives[cur_index].set_primitive_type(QUAD);
    h_primitives[cur_index++].set_quad(point3(0, 555, 0), vec3(555, 0, 0), vec3(0, 0, 555));
    ...
    h_primitives[cur_index].set_translate(vec3(165, 0, 295));
    //for volume
    h_primitives[cur_index++].make_constant_medium(0.01f);
...
__global__ void apply_materials_on_device(int num_objs, primitive* d_primitives, material* d_materials, texture* d_texture) {
    if(threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        d_materials[4].set_tex(&d_texture[4]);
        d_materials[5].set_tex(&d_texture[5]);
        //apply materials
        ...
        d_primitives[6].apply_material(&d_materials[4]);
        d_primitives[7].apply_material(&d_materials[5]);
    }
...
void create_cornell_smoke(int image_width, int image_height) {
    printf("hello create cornell smoke\n");
    //camera setting
    Camera* d_camera;
    printf("start camera init...\n");
    checkCudaErrors(cudaMalloc((void**)&d_camera, sizeof(Camera)));
    camera_init<<<1, 1>>>(d_camera, image_height, image_width, 2048, 5);
    ...
    checkCudaErrors(cudaMalloc((void**)&d_texture, num_objs*sizeof(texture)));
